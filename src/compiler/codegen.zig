...
pub fn emitModule(module: IrModule, writer: anytype) !void {
    try emitModuleOpts(module, false, "chasm_rt.h", null, writer);
}

pub fn emitModuleRaylib(module: IrModule, writer: anytype) !void {
    try emitModuleOpts(module, false, "chasm_rl.h", null, writer);
}

/// Emit an imported module. All C function names are prefixed with
/// `chasm_{module_name}_` to avoid linker collisions with other modules.
pub fn emitModuleImported(module: IrModule, module_name: []const u8, writer: anytype) !void {
    try emitModuleOpts(module, true, "chasm_rt.h", module_name, writer);
}

fn emitModuleOpts(module: IrModule, is_imported: bool, runtime_header: []const u8, module_prefix: ?[]const u8, writer: anytype) !void {
    try writer.print(
    ...
    // ---- Struct typedefs ---------------------------------------------------
    if (module.struct_defs.len > 0) {
        try writer.print("/* Struct types */\n", .{});
        for (module.struct_defs) |sd| {
            try writer.print("typedef struct {{\n", .{});
            for (sd.fields) |f| {
                try writer.print("    {s} {s};\n", .{ cTypeOf(f.type_id, module.struct_defs), f.name });
            }
            try writer.print("}} {s};\n\n", .{sd.name});
        }
        // Typed array helpers for struct element types (skip ChRet_ synthetic structs).
        var any_array_helpers = false;
        for (module.struct_defs) |sd| {
            if (!std.mem.startsWith(u8, sd.name, "ChRet_")) { any_array_helpers = true; break; }
        }
        if (any_array_helpers) try writer.print("/* Typed array helpers */\n", .{});
        for (module.struct_defs) |sd| {
            if (std.mem.startsWith(u8, sd.name, "ChRet_")) continue;
            try writer.print(
                \\static inline {s} chasm_array_get_{s}(ChasmCtx *ctx, ChasmArray *a, int64_t i) {{
                \\    (void)ctx;
                \\    static const {s} zero = {{0}};
                \\    if (i < 0 || i >= a->len) return zero;
                \\    return (({s}*)a->data)[i];
                \\}}
                \\static inline void chasm_array_set_{s}(ChasmCtx *ctx, ChasmArray *a, int64_t i, {s} v) {{
                \\    (void)ctx;
                \\    if (i >= 0 && i < a->len) (({s}*)a->data)[i] = v;
                \\}}
                \\static inline void chasm_array_push_{s}(ChasmCtx *ctx, ChasmArray *a, {s} v) {{
                \\    (void)ctx;
                \\    if (a->len >= a->cap) {{
                \\        a->cap = a->cap * 2 + 8;
                \\        a->data = realloc(a->data, (size_t)a->cap * sizeof({s}));
                \\    }}
                \\    if (a->data) (({s}*)a->data)[a->len++] = v;
                \\}}
                \\
            , .{
                sd.name, sd.name,
    ...
    if (module.attrs.len > 0) {
        ...
        for (module.attrs) |a| {
            try writer.print("static {s} g_{s};\n",
                .{ cTypeOf(a.type_id, module.struct_defs), a.name });
        }
        try writer.print("\n", .{});
    }
    ...
    if (!is_imported) {
        ...
        if (module.attr_init_instrs.len > 0) {
            try emitTempDeclsForSlice(module.attr_init_temps, module.struct_defs, writer);
    ...
    if (module.functions.len > 0) {
        ...
        for (module.functions) |f| try emitForwardDecl(f, module.struct_defs, module.tuple_returns, module_prefix, writer);
        try writer.print("\n", .{});
    }
    ...
    if (module.imported_fwd_decls.len > 0) {
        ...
        for (module.imported_fwd_decls) |fwd| {
            // fwd.name already contains the module prefix (e.g. "utils_add").
    ...
    for (module.functions) |f| try emitFunction(f, module.extern_fns, module.struct_defs, module.tuple_returns, module_prefix, writer);
...
pub fn emitRuntimeHeader(writer: anytype) !void {
    try writer.print(
        ...
        \\
        \\/* ---- string builder ---------------------------------------------- */
        \\typedef struct {{ char *buf; int64_t len; int64_t cap; }} ChasmStrBuilder;
...
fn isMultiReturnFn(name: []const u8, tuple_returns: []const ir.IrTupleReturn) bool {
    for (tuple_returns) |tr| {
        if (std.mem.eql(u8, tr.fn_name, name)) return true;
    }
    return false;
}

fn qualifiedFnName(f_name: []const u8, module_prefix: ?[]const u8, buf: []u8) []const u8 {
    if (module_prefix) |pfx| {
        return std.fmt.bufPrint(buf, "{s}_{s}", .{ pfx, f_name }) catch f_name;
    }
    return f_name;
}

fn emitForwardDecl(f: IrFunction, struct_defs: []const ir.IrStructDef, tuple_returns: []const ir.IrTupleReturn, module_prefix: ?[]const u8, writer: anytype) !void {
    var name_buf: [256]u8 = undefined;
    const qname = qualifiedFnName(f.name, module_prefix, &name_buf);
    if (isMultiReturnFn(f.name, tuple_returns)) {
        try writer.print("ChRet_{s} chasm_{s}(ChasmCtx *ctx", .{ f.name, qname });
    } else {
        const ret = inferReturnType(f);
        try writer.print("{s} chasm_{s}(ChasmCtx *ctx",
            .{ cTypeOf(ret, struct_defs), qname });
    }
    for (f.params) |p| {
        try writer.print(", {s} t{d}", .{ cTypeOf(p.type_id, struct_defs), p.temp_id });
    }
    try writer.print(");\n", .{});
}

fn emitFunction(f: IrFunction, extern_fns: []const ir.IrExternFn, struct_defs: []const ir.IrStructDef, tuple_returns: []const ir.IrTupleReturn, module_prefix: ?[]const u8, writer: anytype) !void {
    var name_buf: [256]u8 = undefined;
    const qname = qualifiedFnName(f.name, module_prefix, &name_buf);
    // Signature
    if (isMultiReturnFn(f.name, tuple_returns)) {
    ...
    try emitTempDeclsForFn(f, struct_defs, writer);
...
fn emitTempDeclsForFn(f: IrFunction, struct_defs: []const ir.IrStructDef, writer: anytype) !void {
    ...
    for (f.temps) |t| {
        ...
        try writer.print("    {s} t{d};\n", .{ cTypeOf(t.type_id, struct_defs), t.id });
    }
    if (f.temps.len > f.params.len) try writer.print("\n", .{});
}

fn emitTempDeclsForSlice(temps: []const Temp, struct_defs: []const ir.IrStructDef, writer: anytype) !void {
    for (temps) |t| {
...
fn emitInstr(instr: Instr, temps: []const Temp, extern_fns: []const ir.IrExternFn, writer: anytype, indent: usize) !void {
    ...
    switch (instr) {
        ...
        .call => |i| {
            ...
            if (isExternFn(i.callee, extern_fns)) |c_name| {
                ...
                const array_builtins = [_][]const u8{
                    ...
                    "str_builder_push", "str_builder_append", "str_builder_build",
                };
                var is_array_builtin = false;
                for (array_builtins) |ab| {
                    if (std.mem.eql(u8, callee, ab)) { is_array_builtin = true; break; }
                }
                // Typed array helpers (array_get_*, array_set_*, array_push_*) also need &.
                if (!is_array_builtin) {
                    if (std.mem.startsWith(u8, callee, "array_get_") or
                        std.mem.startsWith(u8, callee, "array_set_") or
                        std.mem.startsWith(u8, callee, "array_push_"))
                    {
                        is_array_builtin = true;
                    }
                }
                if (i.dest == ir_mod.invalid_temp) {
        ...
        .binary => |i| {
            // String/atom equality must use strcmp, not pointer comparison.
            const l_ty = if (i.left  < temps.len) temps[i.left ].type_id else 0;
            const r_ty = if (i.right < temps.len) temps[i.right].type_id else 0;
            const is_str = (l_ty == 4 or l_ty == 5) and (r_ty == 4 or r_ty == 5); // T_STRING=4, T_ATOM=5
            if (is_str and (i.op == .eq or i.op == .neq)) {
                const cmp = if (i.op == .eq) "==" else "!=";
                try writer.print("{s}t{d} = (t{d} && t{d}) ? (strcmp(t{d}, t{d}) {s} 0) : (t{d} {s} t{d});\n",
                    .{ pad, i.dest, i.left, i.right, i.left, i.right, cmp, i.left, cmp, i.right });
            } else {
                try writer.print("{s}t{d} = t{d} {s} t{d};\n",
        ...
        .ret_tuple => |rt| {
            // Emit: `return (ChRet_fnname){ .v0 = t0, .v1 = t1 };`
            try writer.print("{s}return (ChRet_{s}){{", .{ pad, rt.fn_name });
            for (rt.values, 0..) |t, i| {
                if (i > 0) try writer.print(", ", .{});
                try writer.print(" .v{d} = t{d}", .{ i, t });
            }
            try writer.print(" }};\n", .{});
        },
    }
...
fn cTypeOf(type_id: u32, struct_defs: []const ir.IrStructDef) []const u8 {
    return switch (type_id) {
        ...
        8 => "int64_t",            // T_ENUM_BASE
        9 => "ChasmStrBuilder",    // T_STRBUILDER
        else => {
            for (struct_defs) |sd| {
                if (sd.type_id == type_id) return sd.name;
            }
            return "int64_t";
        },
    };
